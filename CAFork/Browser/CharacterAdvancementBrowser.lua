...
-- SpellTag Mixin 
--
BPCASpellTagMixin = CreateFromMixins("CallbackRegistryMixin")

function BPCASpellTagMixin:OnLoad()
...
function BPCASpellTagMixin:UpdateLayout()
	self:SetWidth(math.max(64, self.Text:GetWidth()+16))
end

function BPCASpellTagMixin:SetTag(value)
	self.filter = nil
	self.tag = value
...
function BPCASpellTagMixin:SetFilter(filter)
	self.tag = nil
	self.filter = filter
...
function BPCASpellTagMixin:OnClick()
...
function BPCASpellTagMixin:OnLeave()
	GameTooltip:Hide()
end
--
-- SpellTag frame
--
BPCASpellTagFrameMixin = CreateFromMixins("CallbackRegistryMixin")

function BPCASpellTagFrameMixin:OnLoad()
	CallbackRegistryMixin.OnLoad(self)
	...
	self.framePool = CreateFramePool("Button", self, "BPCASpellTagTemplate")
	self.framesToID = {}

	self:GenerateCallbackEvents({ "OnSpellTagClicked"})
end

function BPCASpellTagFrameMixin:AllocateButton(button, layoutIndex)
...
function BPCASpellTagFrameMixin:UpdateWidth()
	local rowWidth = 12
...
function BPCASpellTagFrameMixin:OnButtonClick(button)
	self:TriggerEvent("OnSpellTagClicked", button.filter, button.tag)
end

function BPCASpellTagFrameMixin:SetUpSpellTags(filtersTable, spellTagsTable)
...
-- Ability Mixin
--
BPCABrowserAbilityMixin = CreateFromMixins("BPCASpellButtonBaseMixin")

function BPCABrowserAbilityMixin:OnShow()
end

function BPCABrowserAbilityMixin:OnHide()
end

function BPCABrowserAbilityMixin:OnClick(button)
	PlaySound(SOUNDKIT.UCHATSCROLLBUTTON)
	BPCASpellButtonBaseMixin.OnClick(self, button)
end

function BPCABrowserAbilityMixin:OnLoad()
    ...
    self.ClassPoints:SetScript("OnEnter", GenerateClosure(BPCASpellButtonBaseMixin.OnEnterClassPoints, self))
...
function BPCABrowserAbilityMixin:OnEnter()
	if self.isClass then
		...
		BPCASpellButtonBaseMixin.OnEnter(self)
	end
end

function BPCABrowserAbilityMixin:SetDisabledVisual()
	BPCASpellButtonBaseMixin.SetDisabledVisual(self)
	self.Class.Icon:SetVertexColor(0.5, 0.5, 0.5, 0.5)
end

function BPCABrowserAbilityMixin:MakeClass(class, categoryID)
	self.isClass = true
...
function BPCABrowserAbilityMixin:SetEntry(entry, isSuggested)
	self.isClass = false
...
-- browser row mixin
--
BPCABrowserRowMixin = CreateFromMixins(ScrollListItemBaseMixin)

function BPCABrowserRowMixin:OnLoad()
	self:EnableMouse(false)
	self.AbilityPool = CreateFramePool("Button", self, "BPCABrowserSpellButtonTemplate")
...
function BPCABrowserRowMixin:Init()
end

function BPCABrowserRowMixin:SetSelected()
end

function BPCABrowserRowMixin:OnSelected()
end

function BPCABrowserRowMixin:GetScrollParent()
	return self:GetParent():GetParent():GetParent()
end

function BPCABrowserRowMixin:GetAvailableStatus()
	return self.isAvailable
end

function BPCABrowserRowMixin:SetAvailableStatus(isAvailable)
	self.isAvailable = isAvailable
end

function BPCABrowserRowMixin:SetAvailableVisual(isAvailable, text, errorText)
...
function BPCABrowserRowMixin:OnEnterCategoryButton()
...
function BPCABrowserRowMixin:UpdateCategoryVisual()
...
function BPCABrowserRowMixin:LoadIconsOnUpdate()
...
function BPCABrowserRowMixin:SetCategoryID(value)
	self.categoryID = value
end

function BPCABrowserRowMixin:GetCategoryID()
	return self.categoryID
end

function BPCABrowserRowMixin:Update()
...
-- browser mixin
--
BPCABrowserMixin = CreateFromMixins(ScrollListMixin)

function BPCABrowserMixin:BuildCategoryMap()
...
function BPCABrowserMixin:GetNumResults()
	return self.totalRows
end

function BPCABrowserMixin:GetDataForIndex(index) -- returns categoryID, startIndex, maxIndex
	local totalRows = 0
...
function BPCABrowserMixin:OnLoad()
	self.totalRows = 0
	self.categoryMap = {}

	self:SetGetNumResultsFunction(GenerateClosure(self.GetNumResults, self))
	self:SetTemplate("BPCABrowserRowTemplate")
	self:GetSelectedHighlight():SetTexture("") 

	self.ScrollFrame:EnableMouse(false)
end

function BPCABrowserMixin:DisplaySearchResults(searchQuery, filterOutput)
	--dprint("BPCABrowserMixin:DisplaySearchResults")
