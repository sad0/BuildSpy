BPCASpellButtonBaseMixin = {}
BPCASpellButtonBaseMixin.OnEvent = OnEventToMethod

local BROKEN_SPELL_ICON = "Interface\\Icons\\INV_Misc_QuestionMark"

function BPCASpellButtonBaseMixin:OnLoad()
...
function BPCASpellButtonBaseMixin:OnShow()
...
function BPCASpellButtonBaseMixin:OnHide()
...
function BPCASpellButtonBaseMixin:SetEntry(entry)
    if self.Icon.ClassPoints then
...
function BPCASpellButtonBaseMixin:Refresh()
    if self.entry then
...
function BPCASpellButtonBaseMixin:ShowLocation()
    self.Icon.LocationIcon:Show()
end

function BPCASpellButtonBaseMixin:IsBrokenSpell()
...
function BPCASpellButtonBaseMixin:SetBrokenSpellVisual()
...
function BPCASpellButtonBaseMixin:AddBrokenSpellTooltipLines()
...
function BPCASpellButtonBaseMixin:OnEnter()
...
function BPCASpellButtonBaseMixin:OnEnterClassPoints()
    GameTooltip:SetOwner(self, "ANCHOR_RIGHT")
...
function BPCASpellButtonBaseMixin:OnClick(button)
    ...
    if button == "RightButton" then
        BPCharacterAdvancement:ShowSpellDropDownMenu(self)
...
function BPCASpellButtonBaseMixin:Click()
    CloseDropDownMenus()
...
function BPCASpellButtonBaseMixin:OnMouseDown()
    self.Icon:GetPushedTexture():Show()
end

function BPCASpellButtonBaseMixin:OnMouseUp()
    self.Icon:GetPushedTexture():Hide()
end

function BPCASpellButtonBaseMixin:SetDisabledVisual()
    if self.Icon.DisabledOverlay then
...
function BPCASpellButtonBaseMixin:SetEnabledVisual()
    if self.Icon.DisabledOverlay then
...
function BPCASpellButtonBaseMixin:CHARACTER_ADVANCEMENT_LOCK_ENTRY_RESULT(result, internalID)
    if internalID == self.entry.ID then
        self:Refresh()
    end
end

function BPCASpellButtonBaseMixin:CHARACTER_ADVANCEMENT_UNLOCK_ENTRY_RESULT(result, internalID)
    if internalID == self.entry.ID then
        self:Refresh()
    end
end

function BPCASpellButtonBaseMixin:CHARACTER_ADVANCEMENT_PENDING_BUILD_UPDATED()
    if self:IsVisible() then
        self:Refresh()
    end
end

BPCASpellButtonMixin = CreateFromMixins(BPCASpellButtonBaseMixin)

function BPCASpellButtonMixin:OnLoad()
    BPCASpellButtonBaseMixin.OnLoad(self)
...
function BPCASpellButtonMixin:SetEntry(entry)
    BPCASpellButtonBaseMixin.SetEntry(self, entry)
...
function BPCASpellButtonMixin:OnClick(button)
    PlaySound(SOUNDKIT.UCHATSCROLLBUTTON)
    BPCASpellButtonBaseMixin.OnClick(self, button)
end

function BPCASpellButtonMixin:SetDisabledVisual()
    BPCASpellButtonBaseMixin.SetDisabledVisual(self)
end

BPCATalentButtonMixin = CreateFromMixins(BPCASpellButtonBaseMixin)

function BPCATalentButtonMixin:OnLoad()
    BPCASpellButtonBaseMixin.OnLoad(self)
    self:RegisterForDrag("LeftButton")
    self.Icon:SetBorderSize(34, 34)
    self.Icon:SetOverlaySize(34, 34)
...
function BPCATalentButtonMixin:SetRankAndBg(entry, rank, maxRank)
...
function BPCATalentButtonMixin:SetDisabledVisual()
    BPCASpellButtonBaseMixin.SetDisabledVisual(self)
    self.Icon.RankFrame:Hide()
end

function BPCATalentButtonMixin:SetEnabledVisual()
    BPCASpellButtonBaseMixin.SetEnabledVisual(self)
    self.Icon.RankFrame:Show()
end

function BPCATalentButtonMixin:SetEntry(entry)
    BPCASpellButtonBaseMixin.SetEntry(self, entry)
...
function BPCATalentButtonMixin:OnClick(button)
    PlaySound(SOUNDKIT.UCHATSCROLLBUTTON)
    if not BuildCreatorUtil.IsPickingSpells() then
...
BPCAPrimaryStatButtonMixin = CreateFromMixins(BPCASpellButtonBaseMixin)
BPCAPrimaryStatButtonMixin.OnDoubleClick = nop

function BPCAPrimaryStatButtonMixin:SetEntry(entry)
    self.Icon.KnownGlow.Pulse:Stop()

    BPCASpellButtonBaseMixin.SetEntry(self, entry)
end

function BPCAPrimaryStatButtonMixin:SetNoStat()
    self.Icon.KnownGlow:Show()
...
function BPCAPrimaryStatButtonMixin:OnLoad()
    BPCASpellButtonBaseMixin.OnLoad(self)
    self.Icon:SetBorderSize(34, 34)
    self.Icon:SetOverlaySize(34, 34)
    self.Icon.KnownGlow:SetAtlas("ca_known_glow", Const.TextureKit.IgnoreAtlasSize)
...
function BPCAPrimaryStatButtonMixin:OnClick()
    PlaySound(SOUNDKIT.UCHATSCROLLBUTTON)
...
function BPCAPrimaryStatButtonMixin:OnEnter()
...
BPCACompactSpellButtonMixin = CreateFromMixins("BPCASpellButtonBaseMixin")

function BPCACompactSpellButtonMixin:OnLoad()
    BPCASpellButtonBaseMixin.OnLoad(self)
    self:RegisterForDrag("LeftButton")
    self.Icon:SetBorderSize(34, 34)
    self.Icon:SetOverlaySize(34, 34)
...
BPCAMasteryButtonMixin = CreateFromMixins("BPCASpellButtonBaseMixin")

function BPCAMasteryButtonMixin:OnLoad()
    BPCASpellButtonBaseMixin.OnLoad(self)
    self.Icon.KnownGlow:SetAtlas("ca_known_glow", Const.TextureKit.IgnoreAtlasSize)
    self.Icon.KnownGlow:SetVertexColor(NORMAL_FONT_COLOR:GetRGB()) 
end

function BPCAMasteryButtonMixin:OnClick(button)
    PlaySound(SOUNDKIT.UCHATSCROLLBUTTON)
    BPCASpellButtonBaseMixin.OnClick(self, button)
end
