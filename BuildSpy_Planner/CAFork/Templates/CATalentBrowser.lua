...
-- talents mixin
--
BPCATalentsClassMixin = CreateFromMixins(ScrollListItemBaseMixin)

function BPCATalentsClassMixin:OnLoad()
    self.shift = 30
    self.GateCounterTalents = CreateFramePool("Frame", self, "BPCATalentGateCounterTemplate")
    self.TalentPool = CreateFramePool("Button", self, "BPCATalentButtonTemplate")
    self:EnableMouse(false)

    MixinAndLoadScripts(self, "BPCAConnectedNodesMixin")
    MixinAndLoadScripts(self, "BPCATalentFrameGatesMixin")
    --self:SetBackdrop(GameTooltip:GetBackdrop())
end

function BPCATalentsClassMixin:Init()
end

function BPCATalentsClassMixin:SetSelected()
end

function BPCATalentsClassMixin:OnSelected()
end

function BPCATalentsClassMixin:GetScrollParent()
    return self:GetParent():GetParent():GetParent()
end

function BPCATalentsClassMixin:RefreshGateInfoCallBack()
...
function BPCATalentsClassMixin:Update()
    ...
    for i, entry in ipairs(C_CharacterAdvancement.GetTalentsByClass(class, spec, false)) do
        ...
        if BPCharacterAdvancement and (entry.ID == BPCharacterAdvancement.LocateID) then
            button:ShowLocation()
            self.LocateID = nil
        end
...
-- browser mixin
--
BPCATalentBrowserMixin = CreateFromMixins(ScrollListMixin)

function BPCATalentBrowserMixin:UpdateClassCallback(classFile)
    if self.isInBrowserMode then
        return
    end

    BPCharacterAdvancement:SelectClassInNavigation(classFile)
end

function BPCATalentBrowserMixin:LoadElements(caClass, duplicateKnown)
...
function BPCATalentBrowserMixin:OnLoad()
    self:LoadElements()
    self:SetGetNumResultsFunction(function () return #self.specializations end)
    self:SetTemplate("BPCATalentsClassTemplate")
    self:GetSelectedHighlight():SetTexture("") 
end

function BPCATalentBrowserMixin:RefreshGates()
...
function BPCATalentBrowserMixin:SetClass(class)
    if self.isInBrowserMode then
        return
    end
...
function BPCATalentBrowserMixin:GetDataForIndex(index)
...
--[[function BPCATalentBrowserMixin:OnMouseDown()
    self.rotationStart = GetCursorPosition()
    self:SetScript("OnUpdate", self._OnUpdate)
end

function BPCATalentBrowserMixin:OnMouseUp()
    self:SetScript("OnUpdate", nil)
end]]--

function BPCATalentBrowserMixin:_OnUpdate()
...
function BPCATalentBrowserMixin:Init()
