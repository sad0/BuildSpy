BPCABranchMixin = {}
BPCABranchMixin.OnEvent = OnEventToMethod
...
function BPCABranchMixin:Reset()
    self.Left:Hide()
    self.Right:Hide()
    self.Up:Hide()
    self.Down:Hide()
...
function BPCABranchMixin:OnShow()
    self:RegisterEvent("CHARACTER_ADVANCEMENT_PENDING_BUILD_UPDATED")
end

function BPCABranchMixin:OnHide()
    self:UnregisterEvent("CHARACTER_ADVANCEMENT_PENDING_BUILD_UPDATED")
end

function BPCABranchMixin:CHARACTER_ADVANCEMENT_PENDING_BUILD_UPDATED()
    self:Refresh()
end

function BPCABranchMixin:SetDirection(node, enabled)
...
function BPCABranchMixin:SetNode(node, nodeID)
    self.node = node
    self.nodeID = nodeID
    self:Refresh()
end

function BPCABranchMixin:Refresh()
...
function BPCABranchMixin:SetTargetNodeID(nodeID)
    self.nodeID = nodeID
end

function BPCABranchMixin:SetIsEndNode(node)
    local left = node.left
    local right = node.right
    local up = node.up
    local down = node.down
