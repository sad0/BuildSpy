BPCAClassButtonMixin = {}

function BPCAClassButtonMixin:OnLoad()
    self.Overlay:Hide()
    ...
    -- to work with BPCATalentGateCounterMixin
    self.GateText = self.Icon.ClassPoints.Text
end

function BPCAClassButtonMixin:SetClass(classFile)
...
function BPCAClassButtonMixin:UpdateSpellCounts()
    ...
    if C_CVar.GetBool("previewCharacterAdvancementChanges") then
        ...
        if classPoints and (classPoints > 0) then
            ...
            self.tooltipTitle, self.tooltipText = BPCATalentGateCounterMixin.DefineTooltip(self, "CLASS", class)
        else
        ...
        for i = 1, 3 do
            ...
            if tab then
                ...
                if spentOnTab > 0 then
                    ...
                    local icon = BPCATalentGateCounterMixin:DefineIcon("TAB", class, spec)
                    ...
                    button.tooltipTitle, button.tooltipText = BPCATalentGateCounterMixin.DefineTooltip(button, "TAB", class, spec)
                    button:SetScript("OnEnter", BPCATalentGateCounterMixin.OnEnter)
                    button:SetScript("OnClick", function()
                        BPCharacterAdvancement:SelectClass(self.classFile, tab)
                    end)
...
function BPCAClassButtonMixin:OnEnter()
    BPCATalentGateCounterMixin.OnEnter(self)
end

function BPCAClassButtonMixin:OnLeave()
    GameTooltip:Hide()
end

function BPCAClassButtonMixin:OnClick()
    PlaySound(SOUNDKIT.CHARACTER_SHEET_TAB)
    BPCharacterAdvancement:SelectClass(self.classFile, self.topTab and self.topTab.tab or nil)
end 

function BPCAClassButtonMixin:OnSelected()
    self.Overlay:Show()
end

function BPCAClassButtonMixin:OnDeSelected()
    self.Overlay:Hide()
end

function BPCAClassButtonMixin:OnCountLeave()
    GameTooltip:Hide()
